...
class Cnn_net(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(1, 32, 5, stride=(1,1), padding=2),  # 输入通道1，输出通道32，卷积核5x5，步长1，填充2
            nn.ReLU(),                                # ReLU激活函数
            nn.MaxPool2d(kernel_size=2, stride=2)     # 最大池化层，核大小2x2，步长2
        )
        # 第二层内也包含一个卷积层，ReLu激活函数和一个最大池化层
        # 卷积层的输入通道为32，输出通道为64，卷积核大小为5，步长为（1，1），填充值为2；最大池化层和大小为2，步长为2
        self.layer2 = nn.Sequential(
            nn.Conv2d(32, 64, 5, stride=(1,1), padding=2),  # 输入通道1，输出通道64，卷积核5x5，步长1，填充2
            nn.ReLU(),                                # ReLU激活函数
            nn.MaxPool2d(kernel_size=2, stride=2)     # 最大池化层，核大小2x2，步长2
        )
        # 设置正则化层
...
def train(x_train, y_train, x_test, y_test, BATCH_SIZE, model, loss_function, optimizer):
    ...
    print("唐荣泽's procedure has been completed")#将XXX改为你的中文名字，最终提交运行截图
