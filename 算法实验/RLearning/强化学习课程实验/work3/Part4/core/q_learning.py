...
class QN(object):
    ...
    def get_best_action(self, state):
        ...
        # Get Q-values for all actions from the Q-network
        q_values = self.q_network(state_tensor)  # shape: (1, num_actions)
        
        # Get the action with the highest Q-value
        action = torch.argmax(q_values, dim=1).item()  # Get the index of the max Q-value
        return action, q_values.squeeze().cpu().detach().numpy()  # Return action and Q-values
    ...
    def get_action(self, state):
        ...
        if np.random.random() < self.config.soft_epsilon:
            ...
            action,q = self.get_best_action(state)
            return action
