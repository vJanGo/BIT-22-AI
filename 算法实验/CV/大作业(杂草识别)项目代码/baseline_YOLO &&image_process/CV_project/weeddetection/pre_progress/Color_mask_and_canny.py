...
def process_image(input_path, output_path):
    ...
    green_segmented = cv2.bitwise_and(image, image, mask=mask)
    ...
    # 保存绿色分离结果和边缘检测结果
    output_segment_path = os.path.join(output_path, "segmented_" + os.path.basename(input_path))
    output_edge_path = os.path.join(output_path, "" + os.path.basename(input_path))
